...
function CustomersTable(props) {
    ...
    function TotalBills(id){
       ...
       {title: 'Bills',dataIndex: 'bills',sorter: {
        compare: (a, b) => a.bill - b.bill,
      }},{title: 'Details',dataIndex:'details'},
       {title: 'Edit / Remove' ,dataIndex: 'edit' },
       ]
       const Data=()=>{ 
        const result = [...data].reverse().map((cust,i)=>{
        ...
        return {
                ...
                bills:<>{TotalBills(cust._id)}<Link to={`/main/billlist/${cust._id}`}><Breadcrumb.Separator children={"-"}/>view bills</Link></>,
                details:<Link to={`/main/customers/custmodal/${cust._id}`}>View</Link>,
                edit:(<Space size="large" style={{padding:"0 0 0 25px "}}>
                    <EditOutlined onClick={()=>history.push(`/main/customersform/${cust._id}`)}/>
                    <Breadcrumb.Separator/>
                    <Popconfirm
                            title="Are you sure to delete this?"
                            onConfirm={(e)=>{dispatch(removeCustomer(cust._id));message.success("Successfully deleted")}}
