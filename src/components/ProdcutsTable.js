...
function ProdcutsTable(props) {
    ...
    children:[{title: 'Name'  ,dataIndex: 'name'  ,key: 'name'},
             {title: 'Price',dataIndex: 'price',key: 'price',sorter: {
                compare: (a, b) => a.price - b.price,
              }}]},
    {title: 'Details',dataIndex:'details',width:100},{title: 'Edit / Remove' ,dataIndex: 'edit_remove',width:170 },
    ]
    const Data=()=>{ 
        const result = [...data].reverse().map((prod,i)=>{
        return {
                key: i+1,
                name: prod.name,
                price: prod.price,
                details:<Link to={`/main/products/modal/${prod._id}`}>View</Link>,
                edit_remove:<Space size="large" style={{padding:"0 0 0 25px "}}>
                <EditOutlined onClick={()=>history.push(`/main/productsform/${prod._id}`)}/>
                <Breadcrumb.Separator/>
                <Popconfirm
                            title="Are you sure to delete this?"
                            onConfirm={(e)=>{dispatch(removeProduct(prod._id));message.success("Successfully deleted")}}
