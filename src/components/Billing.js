...
function Billing(props) {
  ...
  const billingCustomer = useSelector(
    (state) => state.BusinessData.billingCustomer
  );
  ...
  const Data = () => {
    const result = addProduct.map((product, i) => {
      return {
        ...
        total: `${product.quantity * product.price}/-`.replace(
          /\B(?=(\d{3})+(?!\d))/g,
          ','
        ),
      };
  ...
  return (
    <div>
        ...
        footer={(pageData) => {
          ...
          return (
            <div>
              ...
              >
                <div>
                  <b>Total Product : </b> {totalProducts}
                </div>
                <div>
                  <b>Total Items : </b>
                  {totalItems}
                ...
                <div colSpan='1'>
                  <b>
                    Total Amount : <FontAwesomeIcon icon={faRupeeSign} />{' '}
                  </b>
              ...
              <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
                {!billingCustomer._id || !addProduct.length ? (
                  <Button type='ghost'>Generate Bill</Button>
                ) : (
                  <Button type='primary' disabled={true} onClick={postBill}>
                    Generate Bill
                  </Button>
                )}
              </div>
      ...
      <Route
        path='/:string/:type/billingmodal/:id'
        exact
        component={BillModal}
      />
    </div>
