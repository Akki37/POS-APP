  ...
  const Data = () => {
    ...
    const result = TotalBills(id).map((bill, i) => {
      return {
        ...
        details: (
          <Link to={`/main/billlist/${type}/listmodal/${bill._id}`}>view</Link>
        ),
  ...
  return (
    <>
      <Table
        ...
        bordered
        pagination={false}
        dataSource={customer ? Data(customer._id) : []}
      />
      <Route
        path='/main/billlist/:type/listmodal/:id'
        exact
        component={BillListModal}
      />
    </>
  );
}
