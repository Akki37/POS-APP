...
function CustForminBilling(props) {
  ...
  const { billingCustomer, customers } = useSelector((state) => state.BusinessData.customers);
    ...
    <form onSubmit={handleSubmit}>
      <div className='addCustForm'>
        <div>
          ...
          {formError.mobile ? (
            <p className='error'>
              <WarningOutlined style={{ padding: '0 2px 0 0' }} />
              {formError.mobile}
            </p>
          ) : null}
        </div>
        ...
        <div>
          <Input
            type='email'
            disabled={!mobile.length || !name.length}
            className='customerInputs'
            name='email'
            autoComplete='off'
            placeholder='Email'
